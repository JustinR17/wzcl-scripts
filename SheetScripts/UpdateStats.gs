...
function updateSheet(division) {
  ...
  var res = sheet.getRange(isDivisionC ? "P1:AA314" : "O1:Z254").getValues();
  ...
  for (var row = 0; row < res.length; row++) {
    ...
    if (res[row][1].trim()) {
      if (res[row][1].trim() in players) {
        let targetPlayerTemplate = players[res[row][1].trim()].temp.filter(
          (e) => e.name == templates[templateIndex]
        );
        if (targetPlayerTemplate.length) {
          targetPlayerTemplate[0].wins += res[row][10];
          targetPlayerTemplate[0].losses += res[row][11];
        } else {
          players[res[row][1].trim()].temp.push({
      ...
      if (res[row][4].trim()) {
        if (res[row][4].trim() in players) {
          let targetPlayerTemplate = players[res[row][4].trim()].temp.filter(
            (e) => e.name == templates[templateIndex]
          );
          if (targetPlayerTemplate.length) {
            targetPlayerTemplate[0].wins += res[row][10];
            ...
            players[res[row][4].trim()].temp.push({
              name: templates[templateIndex],
              wins: res[row][10],
              losses: res[row][11],
            });
        ...
        if (res[row][7].trim()) {
          if (res[row][7].trim() in players) {
            let targetPlayerTemplate = players[res[row][7].trim()].temp.filter(
              (e) => e.name == templates[templateIndex]
            );
            if (targetPlayerTemplate.length) {
              targetPlayerTemplate[0].wins += res[row][10];
              ...
              players[res[row][7].trim()].temp.push({
                name: templates[templateIndex],
                wins: res[row][10],
                losses: res[row][11],
              });
...
function writeResults(players, division) {
  ...
  for (var row = 0; row < nameData.length; row++) {
    ...
    if (
      nameData[row][0].trim() == division &&
      name in players &&
      players[name].clan == nameData[row][2].trim()
    ) {
      Logger.log(
        "Template: " +
          template1Data[row][0] +
      ...
      if (
        template1Data[row][0] &&
        playerTemplateDataIndex(players[name].temp, template1Data[row][0]) != -1
      ) {
        let template =
          players[name].temp[
            playerTemplateDataIndex(players[name].temp, template1Data[row][0])
      ...
      if (
        template2Data[row][0] &&
        playerTemplateDataIndex(players[name].temp, template2Data[row][0]) != -1
      ) {
        let template =
          players[name].temp[
            playerTemplateDataIndex(players[name].temp, template2Data[row][0])
          ];
      ...
      if (
        template3Data[row][0] &&
        playerTemplateDataIndex(players[name].temp, template3Data[row][0]) != -1
      ) {
        let template =
          players[name].temp[
            playerTemplateDataIndex(players[name].temp, template3Data[row][0])
          ];
...
function test() {
  ...
  sheet
    .getRange("X3")
    .setValue(
      '=HYPERLINK("https://www.warzone.com/Profile?p=7876548255","jimmy")'
    );
}
